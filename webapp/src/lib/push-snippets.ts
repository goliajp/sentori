...
// Notify sends to every live device the person holds.
func Notify(appUserID, title, body string) error {
...
/// Sends to every live device the person holds.
pub async fn notify(app_user_id: &str, title: &str, body: &str) -> reqwest::Result<()> {
    reqwest::Client::new()
...
public final class Sentori {
    ...
    /** Sends to every live device the person holds. */
    public static void notify(String appUserId, String title, String body) throws Exception {
...
/** Sends to every live device the person holds. */
export async function notify(appUserId: string, title: string, body: string): Promise<void> {
...
def notify(app_user_id: str, title: str, body: str) -> None:
    """Sends to every live device the person holds."""
    payload = json.dumps(
...
public static class Sentori
{
    ...
    /// <summary>Sends to every live device the person holds.</summary>
    public static async Task NotifyAsync(string appUserId, string title, string body)
    {
...
// Sends to every live device the person holds.
bool Notify(const std::string& app_user_id, const std::string& title,
