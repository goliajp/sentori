...
export const en = {
  ...
  'push.audienceAlreadySent': 'Already sent to these — count again to send another',
  ...
  'push.ready': 'Everything push needs is configured.',
  'push.readyWithDevices': 'Configured, with {n} devices reachable.',
  'push.level.blocked': 'blocked',
  'push.level.warn': 'gap',
  'push.level.info': 'note',
  'push.check.no-device': 'No device has registered.',
  'push.fix.no-device': 'The app has not called sentori.push.register(), or every user declined the prompt. {quarantined} devices were retired.',
  'push.check.no-credential': '{devices} devices are on {provider}, and there is no {provider} credential.',
  'push.fix.no-credential': 'Nothing can be delivered to them until one is saved.',
  'push.check.queue-stalled': '{queued} sends have been queued for {minutes} minutes.',
  'push.fix.queue-stalled': 'The worker drains every 5 seconds, so this queue is not being read. Check the server is running with push enabled.',
  'push.check.no-identity': 'None of the {live} devices knows who holds it.',
  'push.fix.no-identity': 'Call sentori.user() in the app. Without it, sending to a person or to an issue reaches nobody — and answers 200.',
  'push.check.no-traits': 'None of the {live} devices carries any traits.',
  'push.fix.no-traits': 'Pass them to sentori.user({ id, traits }). Conditions on plan, locale or cohort match nothing until you do.',
  'push.check.no-metadata': 'None of the {live} devices reported anything about itself.',
  'push.fix.no-metadata': 'Pass metadata to push.register({ metadata }). Conditions on app version or build channel match nothing until you do.',
  'push.check.credential-unused': 'A {provider} credential is saved and no {provider} device has registered.',
  'push.fix.credential-unused': 'Harmless, unless you expected devices on it — then the app is registering as something else.',
  'push.check.mass-quarantine': '{quarantined} devices are retired and only {live} are live.',
  'push.fix.mass-quarantine': 'That ratio is usually a credential aimed at the wrong app or the wrong Apple host, not devices going away.',
  'push.check.all-failing': 'All {failed} sends in the last day failed.',
  'push.fix.all-failing': 'The vendor said: {reason}',
  'push.check.apns-mixed-env': '{sandbox} of {live} devices are on Apple sandbox.',
  'push.fix.apns-mixed-env': 'Debug and store builds live on different Apple hosts. Each device is delivered to its own — one key covers both.',
