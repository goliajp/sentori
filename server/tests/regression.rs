...
/// v2.1 — releasing this test from its previous "cross-release
/// regression flip" assertion. After 2026-05-22 per-release
/// isolation policy (release IS in fingerprint), a new release
/// with the same exception creates a NEW issue rather than flipping
/// the resolved one back to regressed. The dashboard's
/// related-issues panel surfaces "this looks like a fixed issue
/// from 1.0" so triage doesn't lose visibility — but the rows
/// stay separate.
///
/// The in-release regression flip (same release re-ingest of a
/// resolved issue) is still covered by `re_resolving_clears_*`
/// below — that one resolves + re-ingests on the SAME release, so
/// the issue does re-open.
#[tokio::test]
async fn cross_release_event_creates_new_issue_not_regression() {
    ...
    // 1. First event in 1.0 creates issue A.
    ...
    let issues: Vec<Value> = Client::new()
        ...
        .await
        .unwrap();
    let issue_a = issues[0]["id"].as_str().unwrap().to_string();

    // 2. Resolve A.
    let r = Client::new()
        .patch(format!(
            "http://{addr}/admin/api/projects/{project_id}/issues/{issue_a}"
        ))
        .header("cookie", &cookie)
    ...
    // 3. Ingest the same exception in a LATER release. Under the
    //    v2.1 per-release-isolation policy this creates a brand-new
    //    issue B — A stays resolved.
    ingest(&addr, &token, "myapp@1.1.0").await;

    // A: still resolved (NOT regressed).
    let body_a: Value = Client::new()
        .get(format!(
            "http://{addr}/admin/api/projects/{project_id}/issues/{issue_a}"
        ))
        .header("cookie", &cookie)
        ...
        .unwrap();
    assert_eq!(
        body_a["status"].as_str().unwrap(),
        "resolved",
        "post-v2.1, 1.0's issue stays resolved when 1.1 emits the same exception"
    );

    // Issue list: two distinct rows. The new event's row must be
    // present and active.
    let issues_after: Vec<Value> = Client::new()
        .get(format!(
            "http://{addr}/admin/api/projects/{project_id}/issues?status=active"
        ))
        .header("cookie", &cookie)
        .send()
        .await
        ...
        .await
        .unwrap();
    let issue_b_present = issues_after
        .iter()
        .any(|i| i["id"].as_str().unwrap() != issue_a);
    assert!(
        issue_b_present,
        "a new issue row should exist for the 1.1 release event"
    );
}
