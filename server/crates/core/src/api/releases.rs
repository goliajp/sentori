...
pub struct ReleaseRow {
    ...
    /// v2.4 — distinct identity fingerprints that hit an error/anr/
    /// nearCrash event under this release. Mobile vitals' canonical
    /// "affected users" measure. Always present; zero when no
    /// identified users have errored on this release.
    #[serde(default)]
...
pub async fn list_releases(
    ...
    let rows: Vec<ReleaseRow> = sqlx::query_as(
        ...
        SELECT
            ...
            COALESCE(au.affected_users, 0)::bigint AS affected_users
        FROM releases r
        LEFT JOIN (
            SELECT release_id,
