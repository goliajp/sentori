...
pub async fn check_and_record(
    ...
    let ttl_s = COUNTER_TTL.whole_seconds();
    if current >= limit_u64 {
        // v1.1 P1: atomic INCR+TTL.
        let _ = crate::valkey::incr_with_ttl(&mut conn, &dropped_key, 1, ttl_s).await?;
    ...
    let new_count =
        crate::valkey::incr_with_ttl(&mut conn, &usage_key, 1, ttl_s).await? as u64;
