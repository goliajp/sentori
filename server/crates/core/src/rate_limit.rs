...
async fn check_rate_limit(
    ...
    // v1.1 P1: atomic INCR + (EXPIRE iff new). Replaces the
    // separate INCR/EXPIRE pair that could leak keys without TTL
    // if the connection dropped between calls.
    let count = crate::valkey::incr_with_ttl(&mut conn, &key, 1, 60).await? as u32;
