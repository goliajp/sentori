...
///
/// v2.0 — message-kind events have no error object; we silently skip
/// symbolication for them (nothing to symbolicate).
pub fn symbolicate_event_with_map(sm: &SourceMap, event: &mut Event) {
    if let Some(err) = event.error.as_mut() {
        symbolicate_error_object(sm, err);
    }
}
