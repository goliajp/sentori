...
/// Full URL provider should redirect to after the user grants
/// access. Must match what's registered in the provider's OAuth
/// app config (Google Cloud Console / GitHub OAuth App).
///
/// The path is `/auth/oauth/<provider>/callback` — without `/api/`
/// — even though the actual axum route is mounted at
/// `/api/auth/oauth/<provider>/callback` (see router.rs's
/// `.nest("/api/auth", user_auth)`). The reason: Caddy on the
/// origin (devops/devices/cloud/t01/caddy/Caddyfile) has a
/// rewrite `handle /auth/* { uri replace /auth/ /api/auth/ }`
/// that pushes the public-facing short URL into the canonical
/// route. This keeps the URL the end-user sees in the OAuth
/// consent screen short (no `/api/` clutter), and lets the
/// callback URLs registered in Google Cloud Console + GitHub
/// OAuth App stay short.
///
/// Migrating the host (e.g. `app.sentori.golia.jp` →
/// `sentori.golia.jp` for v2.4) does NOT require changing this
/// path — the new Caddyfile keeps the same rewrite.
...
pub async fn callback(
    ...
    // v2.4 — post-OAuth-success redirect to `/main` (was `/`).
    // After the v2.4 single-domain consolidation, `/` is the
    // marketing landing; logged-in users land on `/main`, where
    // RootRedirect picks the user's first org's Overview.
    (
