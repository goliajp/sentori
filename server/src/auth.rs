...
pub async fn require_token(
    ...
    let Some(token) = token else {
        return crate::error::err_response_with(
            StatusCode::UNAUTHORIZED,
            "auth.missingToken",
            "missing Authorization: Bearer <token> header",
            Some("send `Authorization: Bearer <st_pk_… token>` — find it in the dashboard under project settings → tokens".to_string()),
            Some("https://sentori.golia.jp/docs/errors/auth.missingToken".to_string()),
            "auth",
            vec![],
        );
    };
    ...
    let Some(caller) = state.resolve(token).await else {
        return crate::error::err_response_with(
            StatusCode::UNAUTHORIZED,
            "auth.invalidToken",
            "token not recognised for this project",
            Some(token_hint(token).to_string()),
            Some("https://sentori.golia.jp/docs/errors/auth.invalidToken".to_string()),
            "auth",
            vec![],
        );
    };
