...
export function AlertsView() {
  ...
  if (error)
    return <div className="px-6 py-6 text-sm text-red-400">Failed to load alert rules.</div>
      ...
      <header className="flex items-baseline justify-between">
        <div>
          ...
          <p className="text-fg-muted mt-1 text-[12px]">
            Trigger emails when issues land, regressions appear, event volume spikes, or crash-free
            rate dips.
          </p>
        </div>
        {canManage && (
