...
PORT="${SENTORI_LIVE_PORT:-8393}"
PGPORT="${SENTORI_LIVE_PGPORT:-55434}"
...
cleanup() {
    ...
    docker rm -f sentori-live-pg-android >/dev/null 2>&1 || true
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    docker rm -f sentori-live-pg-android >/dev/null 2>&1 || true
    docker run -d --name sentori-live-pg-android \
        -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=dev -e POSTGRES_DB=sentori \
        -p "${PGPORT}:5432" postgres:18-alpine >/dev/null
    for _ in $(seq 1 60); do
        docker exec sentori-live-pg-android pg_isready -U postgres >/dev/null 2>&1 && break
        sleep 1
    done
