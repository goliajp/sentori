...
description: Wrap captureException around Redux, Zustand, and TanStack Query
---

# State management

Three idiomatic ways to wire `captureException` into the most common
    ...
    initializer(
      (partial, replace) => {
        try {
          ...
          captureException(err as Error, { tags: { source: 'zustand.set' } })
          throw err
        }
      },
      get,
...
All three patterns above call `captureException` **once** per error and
