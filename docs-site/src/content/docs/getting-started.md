...
All optional. Existing v1.x code keeps working. Pick what's
useful.

### Cross-project user lookup

If your users are in multiple projects within one org, Sentori
can join their events across projects — without ever storing
raw email / phone / OAuth sub:
...
In the dashboard, the **Users** page lets an operator paste a
real email, browser-side hashes it, and the URL/query only ever
carries the hash. See [Privacy & identity](./privacy/identity.md).

### Silent SDK + structured ready signal

Sentori is silent on the host's console by default. To know
...
`captureMessage`, `startTrace`, `startSpan` / `withScopedSpan`,
`startMoment`, `recordMetric`, `track`, `addBreadcrumb`. See
the [recipes](./recipes/) directory:

- [manual-issue.md](./recipes/manual-issue.md) — `captureMessage`
- [manual-trace.md](./recipes/manual-trace.md) — top-level traces
- [manual-span.md](./recipes/manual-span.md) — child spans
- [manual-moment.md](./recipes/manual-moment.md) — funnels
- [manual-breadcrumb.md](./recipes/manual-breadcrumb.md) — context trail
- [track-and-metrics.md](./recipes/track-and-metrics.md) — analytics + numeric
...
See [Sentry compat](./sentry-compat.md).
...
- [Migrating to v2.3](./migration-v1-to-v2.md) — what changed
  between v1.x and v2.x SDKs (everything is additive — no
  required code changes)
- [Privacy & identity](./privacy/identity.md) — what Sentori
  sees / doesn't see / can't recover. Audit-safe by construction.
- [Sentry compat](./sentry-compat.md) — drop-in `Sentry.*` API
  surface mapping into Sentori-native
