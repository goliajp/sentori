...
- [`sdk-host-app-impact.md`](./performance/sdk-host-app-impact.md) —
  SDK-side budget targets (main-thread occupancy, frame drop diff,
  replay attachment size, screenshot size) + manual verification
  rig for iOS sim-sentori / Android Pixel 10 Pro AVD before each
  publish. The "几乎不能造成性能抖动" mandate operationalised.
