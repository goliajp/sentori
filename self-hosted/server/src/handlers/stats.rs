...
pub async fn project_stats(
    ...
    let events: i64 = scalar_count(
        &state,
        "SELECT COUNT(*)::bigint FROM events WHERE project_id = $1 AND received_at >= now() - interval '24 hours'",
        project_id,
    )
    .await?;
    let issues_active: i64 = scalar_count(
        &state,
        "SELECT COUNT(*)::bigint FROM issues WHERE project_id = $1 AND status = 'active'",
        project_id,
    )
    .await?;
    let spans: i64 = scalar_count(
        &state,
        "SELECT COUNT(*)::bigint FROM spans WHERE project_id = $1 AND received_at >= now() - interval '24 hours'",
        project_id,
    )
    .await?;
    ...
    let replays: i64 = scalar_count(
        &state,
        "SELECT COUNT(*)::bigint FROM replay_sessions WHERE project_id = $1 AND created_at >= now() - interval '24 hours'",
        project_id,
    )
    .await?;
    Ok(Json(json!({
