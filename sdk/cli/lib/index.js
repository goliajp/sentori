...
async function cmdUploadSrcbundle(argv) {
    ...
    try {
        ...
        console.log(`uploaded a source bundle for "${c.release}" — ${stats.files} file(s)` +
        ...
        return lenientFail(strict, {
            failure: `srcbundle upload failed (${e.message})`,
...
async function cmdMcpServe(argv) {
    ...
    try {
        ...
        console.error(`mcp serve failed: ${e.message}`);
        return 1;
    }
}
// ── the release gate ──────────────────────────────────────────────
async function cmdArtifactsCheck(argv) {
    let parsed;
    try {
        parsed = parseArgs({
            allowPositionals: true,
    ...
    try {
        ...
        console.error(`artifacts check failed: ${e.message}`);
        return 1;
    }
    if (!res.known) {
        console.error(`Sentori has never seen the release "${cfg.release}".\n` +
    ...
    for (const a of res.artifacts.filter((x) => x.usable === false)) {
        console.error(`unreadable ${a.kind}: ${a.name}\n` +
    ...
    const expect = typeof parsed.values.expect === 'string'
    ...
    console.error(`missing for ${cfg.release}: ${absent.join(', ')}\n` +
...
async function main(argv) {
    ...
    if (a === 'upload' && b === 'srcbundle')
        return cmdUploadSrcbundle(rest);
    if (a === 'react-native' && b === 'upload')
        return cmdReactNativeUpload(rest);
    if (a === 'artifacts' && b === 'check')
        return cmdArtifactsCheck(rest);
    if (a === 'probes' && b === 'sync')
    ...
    if (a === 'push' && b === 'receipt')
        return cmdPushReceipt(rest);
    if (a === 'push' && b === 'creds') {
        const [c, ...rest2] = rest;
        if (c === 'list')
            return cmdPushCredsList(rest2);
        if (c === 'set')
            return cmdPushCredsSet(rest2);
        if (c === 'delete')
            return cmdPushCredsDelete(rest2);
    }
...
// ── push commands (v2.12) ─────────────────────────────────────────
async function cmdPushSend(argv) {
    ...
    try {
        const data = parsed.values.data ? parseJsonArg(parsed.values.data, '--data') : undefined;
        const priority = parsed.values.priority;
...
async function cmdPushReceipt(argv) {
    const parsed = parseArgs({
        args: argv,
        options: {
            'api-url': { type: 'string' },
    ...
    try {
        ...
        console.error(`push receipt failed: ${e.message}`);
        return 1;
    }
}
async function cmdPushCredsList(argv) {
    const parsed = parseArgs({
        args: argv,
        options: {
            'api-url': { type: 'string' },
    ...
    try {
        ...
        console.error(`push creds list failed: ${e.message}`);
        return 1;
    }
}
async function cmdPushCredsSet(argv) {
    const parsed = parseArgs({
        args: argv,
        options: {
            'api-url': { type: 'string' },
    ...
    try {
        const config = parseJsonArg(parsed.values.config, '--config');
        const secret = parseJsonArg(parsed.values.secret, '--secret');
        await pushCredsSet(cfg, provider, config, secret);
        console.log(`${provider} ✓ saved`);
        return 0;
    }
    catch (e) {
        console.error(`push creds set failed: ${e.message}`);
        return 1;
    }
}
async function cmdPushCredsDelete(argv) {
    const parsed = parseArgs({
        args: argv,
        options: {
            'api-url': { type: 'string' },
    ...
    try {
        ...
        console.error(`push creds delete failed: ${e.message}`);
        return 1;
    }
}
