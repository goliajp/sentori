...
export const configureRing = (opts: { capacity?: number; windowMs?: number }): void => {
  if (opts.capacity && opts.capacity > 0) capacity = opts.capacity
  if (opts.windowMs && opts.windowMs > 0) windowMs = opts.windowMs
}

/** Push one signal. O(1); oldest entry drops past capacity. */
export const pushSignal = (kind: string, data?: Record<string, unknown>): void => {
 ...
 */
export const snapshotSignals = (now: number = Date.now()): Signal[] => {
