...
> 一个 RN-first、Sentry 替代、完全自研协议的 APM。**Self-host only。**
> 后端:Rust + axum + PostgreSQL 18+。前端:`webapp/`(React 19 + Vite + Tailwind v4,全 SPA)。
> 没有 KV / 缓存层 —— Valkey 随 v0.1 栈在 2026-07-23 删除,之后没有替代。

## 当前状态

> 版本以仓库根的 [`VERSION`](./VERSION) 为准,产品面的实际变化逐版记在 [`CHANGELOG.md`](./CHANGELOG.md) 顶部。
>
> 这里以前挂着一段自称「当前」的快照,写的是 2026-06-23 的 v0.2 pivot —— SaaS 双 binary、
> 待拍板的 cutover、一份 v0.2 的 surface 清单。cutover 2026-07-20 就执行了,SaaS 面在 v1
> 重构里连同十六个 crate 一起删了,而这段文字又活了两个半月,读起来仍然像是在描述今天。
>
> 一份手写的功能清单会用和代码一样的速度腐烂,而且腐烂时看不出来。所以这里不再维护清单:
> 要知道现在有什么,读 CHANGELOG 顶部几条,或者直接看 `webapp/src/pages/` 和
> `self-hosted/server/src/handlers/`。
...
- **v2.34** ⏳ in progress 2026-06-10 — **Push Phase 2 第十五笔:preference center API**。Migration 0082 加 `push_preferences(project_id, user_fingerprint_hex, category, opted_out)` 复合 PK。`NativeMessage.preferenceCategory: Option<String>` 新 wire 字段(区别于 `NativeOptions.category` 那个 iOS aps.category)。`enqueue_one` 在 device 有 `user_fingerprint_hex` AND message 有 `preferenceCategory` 时查 preference,opted_out=true 返 synthetic ticket `status=failed, providerOutcome=PreferenceOptedOut`(silent skip 而非 batch failure)。Endpoints:`PUT /v1/push/users/:fpHex/preferences/:category {optedOut}` upsert;`GET /v1/push/users/:fpHex/preferences` list。**API + 表 only,不做 hosted end-user UI**(整 series 一致)。102 push 单测 + preflight 全绿。详见 [`docs/roadmap/v2.34.md`](./docs/roadmap/v2.34.md)。
- **v2.35** ⏳ in progress 2026-06-10 — **Push Phase 2 第十六笔(收官):dispatch sweep `FOR UPDATE SKIP LOCKED` for multi-worker safety**。`dispatch_cron::sweep_once` 把 SELECT 包进 CTE:`WITH claimed AS (SELECT id FROM push_sends WHERE status='queued' AND next_attempt_at<=now() ORDER BY next_attempt_at LIMIT $1 FOR UPDATE SKIP LOCKED) SELECT … FROM claimed cl JOIN push_sends s ON s.id=cl.id …`。单实例行为不变(SKIP LOCKED 对独 worker 是 no-op);多实例直接 ready,sibling worker 的 CTE 跳过已锁行,不会双发。零 schema/dep 改。102 push 单测 + preflight 全绿。详见 [`docs/roadmap/v2.35.md`](./docs/roadmap/v2.35.md)。
- **v2.36** ⏳ in progress 2026-06-10 — **Push Phase 2 webapp polish:把 v2.25–v2.34 server 字段全 UI 承接**。`admin_get_push_send_detail` 扩 SELECT(新 `acked_at / ack_session_id / campaign_id / template_id / audience_tag / preferenceCategory`),18 列改用 `#[derive(sqlx::FromRow)]`。`admin_list_push_sends` 加 `acked_at + campaign_id`。Web:`PushSendDetail.send + PushSendRow` 类型扩;send-detail-view 加 `DeliveryAckCard`(v2.26 ack)、`CampaignTagsCard`(v2.25 BI + v2.34 preference category)+ envelope card `⏰ scheduled` badge(v2.32);SendsTab 加 `Ack`(✓/—)+ `Campaign` 两列。零新 endpoint;现有 query key / fetch hook 不变;旧字段全 nullable 兼容老 server。102 push 单测 + preflight 全绿。详见 [`docs/roadmap/v2.36.md`](./docs/roadmap/v2.36.md)。
- **v2.37** ⏳ in progress 2026-06-10 — **Push Phase 2 adaptive throttle + global per-provider circuit-breaker**。关掉 Insight team 在 push 评审中提的两个洞:#1 没有 429 驱动的 L1 dial-back;#3 没有全局 per-provider 熔断;v2.23 留的 should_auto_throttle 只 warn 不作用 bug 也顺带修。`TokenBucket` 加 `Option<(factor, until)>` 节流字段,`try_acquire` 用 `factor*refill_per_sec` 算有效速率,过期自动清零。`HealthState::should_global_throttle(kind)` 跨所有 project 聚合 transient 比例,≥20% AND ≥50 sample 触发。`RateLimiter::throttle_provider/throttle_project` 半速 5min。dispatch_cron 在 health.record 之后:InvalidToken 命中 → throttle_project(L2);Transient 命中 global threshold → throttle_provider(L1)。默认 50% × 5min。零 wire / SDK / dashboard 改。109 push 单测(7 新)+ preflight 全绿。详见 [`docs/roadmap/v2.37.md`](./docs/roadmap/v2.37.md)。
