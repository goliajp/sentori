...
- **v2.7 → v2.12** 🚧 **Push notifications** — multi-version rollout 启动 2026-06-07。架构冻结在 [`docs/design/push-architecture.md`](./docs/design/push-architecture.md):5 层(SDK / server / push module / provider trait / Postgres),5 provider(APNs/FCM/Web Push/HCM/MiPush),Sentori-native wire format + Expo-compat 端点,project-scoped credential 用新 `server/src/secrets.rs` (AES-256-GCM) 加密落库。**v2.7** ✅ shipped 2026-06-07 — server foundation:`server/src/secrets.rs` AES-256-GCM (HKDF-SHA256 from `SENTORI_SESSION_SECRET`) + 4 migrations (`device_tokens`/`push_credentials`/`push_sends`/`push_delivery_logs`) + `server/src/push/` module tree (providers + dispatch_cron + tokens/send/delivery + expo_compat) + 完整 APNs provider (ES256 JWT + HTTP/2 + 错误分类) + 完整 FCM v1 provider (RS256 JWT-bearer OAuth + access token cache + 错误分类) + dispatch cron (30s tick mirrors `webhook_dispatch`,JOIN push_sends⋈device_tokens⋈push_credentials,decrypt,fan-out,retry [60s,5m,30m,2h,12h,24h]×6,bad-token streak threshold 3 自动 revoke) + `/v1/push/{tokens,send,receipts,expo-compat/*}` 6 个 routes + `admin/api/projects/{id}/push/credentials` CRUD (encrypted 写入,GET 不返回 secret) + AppState 加 shared `reqwest::Client`。146 lib tests pass (新增 19 个 push tests),零 SDK 改动。详见 [`docs/roadmap/v2.7.md`](./docs/roadmap/v2.7.md)。**v2.8** Web Push (`sentori-javascript` + VAPID + `sentori-next` server send + recipe);**v2.9** RN iOS native module(APNs delegate);**v2.10** RN Android native module(FCM);**v2.11** `sentori-expo` config plugin + dashboard `push` 模块(chord `g n`, group `manage`);**v2.12** HCM + MiPush + framework hooks + perf bench。
