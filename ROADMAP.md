...
- **v2.2** 🚧 in-flight 2026-06-03 — find-bug lens dashboard 重设计：`/explore` 单查询端点（`dim × measure × filter` 白名单，UI 和 LLM agent 共用一个 query 形状）+ 把 Issues / Releases 模块改成 `/explore` 消费者。W1（端点）+ W2（Releases list+detail）已在 prod；W3（Issues list 改写）+ W4（doc recipe + closeout）pending。零 SDK 改动 —— v2.2 是纯 read-side dashboard 工作。其它隐藏模块（traces / metrics / vitals / moments / audience / cert-monitor / posture / privacy / live-debug / alerts）继续 hidden 等待各自的 lens（find-slow / find-user / find-threat / engineering-hygiene）在 v2.3+ 开。详见 [`docs/roadmap/v2.2.md`](./docs/roadmap/v2.2.md)。
