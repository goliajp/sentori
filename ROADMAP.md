...
- **v0.7** ✅（pending publish）综合升级：Phase 43 Linear/Slack 双向集成 + Phase 44 Sampling/数据分析 + Phase 45 Web SDK 矩阵（Vue/Svelte/Solid 三个新包） + Phase 46 Session-trail 轻量回放 + Phase 47 polish（Related issues panel、perf bench、CHANGELOG）。剩 Insight 验证 → 一次性 npm publish + v0.7 git tag
...
- [ ] E.01 SDK bump：core 0.6.0、javascript 0.4.0、react-native 0.7.0、vue/svelte/solid 0.1.0 — 与 v0.7 整体 publish 一起做（Phase 47.07）
- [x] E.02 docs：sessionTrail 章节加在 docs/sdk-react-native.md（"Session trail (opt-in)"），mirror 到 docs-site
- [ ] E.03 Insight 通知（v0.7 整体 publish 后一次性发）
...
- [ ] 47.02 G.10 hover frame ↔ tree node 联动 — deferred，单独 follow-up（需要把 viewTree 节点路径映射回 stack frame 的 file:line，data model 改动有限但联动 UI 体积不小，单独成 phase 更妥）
...
- [ ] 47.05 Dashboard LCP < 1.2s gate (lighthouse-style snapshot) — deferred，需要 Lighthouse CI infra（容器 + 头部浏览器），单独走
- [x] 47.06 ROADMAP + CHANGELOG v0.7 完整 summary（CHANGELOG.md 已加 "v0.7 — 综合升级" 大段，Phase 43 → 47 每个 sub 都有交付凭证 commit hash）
- [ ] 47.07 v0.7 tag + GitHub release — 等 Insight 验证后一次性走（包括 7 个 npm 包 publish 0.4.0/0.6.0/0.7.0/0.1.0）
